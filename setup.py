...
eggFile = 'AI_MutiTensor-' + gpaVer + "-py" + str(sys.version_info[0]) + '.' + str(sys.version_info[1]) + '.egg'
...
def uninstall():
    ...
    for x in sys.path:
        ...
        print(eggPath)
        print(x.lower())
        if x.lower().find('ai_mutitensor-') > 0 and x.lower().find('dist-packages') > 0 :
            gpaPath.append(x)
            print(x)

    if (len(gpaPath) == 0):
        print(u'AI_MutiTensor！！！')
        return
    ...
    # 卸载命令
    # result = os.popen('easy_install --m AI_MutiTensor')
...
if len(sys.argv) >= 2:
    if (sys.argv[1] == 'install'):
        ...
        print(u'开始卸载旧版本AI_MutiTensor安装包')
        uninstall()
